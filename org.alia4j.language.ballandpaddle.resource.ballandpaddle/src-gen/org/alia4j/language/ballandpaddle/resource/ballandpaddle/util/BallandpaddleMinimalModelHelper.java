 ...
 */
public class BallandpaddleMinimalModelHelper {
	
	private final static org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddleEClassUtil eClassUtil = new org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddleEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddleStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddleListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddleStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddleListUtil.castListUnchecked(value);
