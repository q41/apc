...
public class BallandpaddleDefaultNameProvider implements org.alia4j.language.ballandpaddle.resource.ballandpaddle.IBallandpaddleNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddleEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
