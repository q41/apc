...
public class BallandpaddleDebugThread extends org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.BallandpaddleDebugElement implements org.eclipse.debug.core.model.IThread, org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.IBallandpaddleDebugEventListener {
	
	private boolean suspended = false;
	private org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.BallandpaddleDebugTarget debugTarget;
	
	public BallandpaddleDebugThread(org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.BallandpaddleDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.BallandpaddleDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddleStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.BallandpaddleStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.BallandpaddleDebugTarget getTarget() {
		return debugTarget;
	}
