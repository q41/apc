...
public abstract class AbstractBallandpaddleDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddlePair<String, Integer>> lineBreakpoints = new java.util.ArrayList<org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddlePair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddleRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddleRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.EBallandpaddleDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.BallandpaddleDebugMessage message = new org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.BallandpaddleDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddlePair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.EBallandpaddleDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.EBallandpaddleDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddlePair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new org.alia4j.language.ballandpaddle.resource.ballandpaddle.util.BallandpaddlePair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.EBallandpaddleDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(org.alia4j.language.ballandpaddle.resource.ballandpaddle.debug.EBallandpaddleDebugMessageTypes.STARTED, true);
			setSuspend(true);
