...
public class TextadventureDefaultNameProvider implements org.alia4j.language.textadventure.resource.textadventure.ITextadventureNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.alia4j.language.textadventure.resource.textadventure.util.TextadventureEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
