...
/**
 * The TextadventureTaskItemDetector is used to find task items in text documents.
 * The current implementation searches for specific keywords to detect task items.
 * The TextadventureTaskItemDetector is used both by the TaskItemBuilder and the
 * editor.
 */
public class TextadventureTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.alia4j.language.textadventure.resource.textadventure.mopp.TextadventureTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.alia4j.language.textadventure.resource.textadventure.mopp.TextadventureTaskItem> foundItems = new java.util.ArrayList<org.alia4j.language.textadventure.resource.textadventure.mopp.TextadventureTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.alia4j.language.textadventure.resource.textadventure.mopp.TextadventureTaskItem(keyword, message, localLine, offset, end));
