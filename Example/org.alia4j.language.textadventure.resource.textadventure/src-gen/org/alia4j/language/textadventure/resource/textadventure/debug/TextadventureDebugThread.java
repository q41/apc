...
public class TextadventureDebugThread extends org.alia4j.language.textadventure.resource.textadventure.debug.TextadventureDebugElement implements org.eclipse.debug.core.model.IThread, org.alia4j.language.textadventure.resource.textadventure.debug.ITextadventureDebugEventListener {
	
	private boolean suspended = false;
	private org.alia4j.language.textadventure.resource.textadventure.debug.TextadventureDebugTarget debugTarget;
	
	public TextadventureDebugThread(org.alia4j.language.textadventure.resource.textadventure.debug.TextadventureDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			org.alia4j.language.textadventure.resource.textadventure.debug.TextadventureDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = org.alia4j.language.textadventure.resource.textadventure.util.TextadventureStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new org.alia4j.language.textadventure.resource.textadventure.debug.TextadventureStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public org.alia4j.language.textadventure.resource.textadventure.debug.TextadventureDebugTarget getTarget() {
		return debugTarget;
	}
